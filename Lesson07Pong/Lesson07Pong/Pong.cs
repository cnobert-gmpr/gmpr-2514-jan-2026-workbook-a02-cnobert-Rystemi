...
public class Pong : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        // bounce the ball off the left and right sides
        if (_ballPosition.X <= PlayAreaBoundingBox.Left || _ballPosition.X >= PlayAreaBoundingBox.Right - _BallWidthAndHeight)
        {
            _ballDirection.X *=-1;
        }

        if (_ballPosition.Y <= PlayAreaBoundingBox.Top +_PlayAreaEdgeLineWidth  || _ballPosition.Y >= PlayAreaBoundingBox.Bottom - _BallWidthAndHeight - _PlayAreaEdgeLineWidth)
        {
            _ballDirection.Y *=-1;
        }
